#ifndef __AIRTSP_SERVICEABSTRACT_HPP
#define __AIRTSP_SERVICEABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const AIRTSP::ServiceAbstract& iService) {
  /**
     string stream:
      - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            AIRTSP::ServiceAbstract& ioService) {
  // Fill Service object with input stream
  ioService.fromStream (ioIn);
  return ioIn;
}

#endif // __AIRTSP_SERVICEABSTRACT_HPP
